...
import { hostOrigin, pickArtifact } from '../host.tsx';
...
function PushLocalArtifact(): VNode {
  ...
  const browse = async () => {
    // a web console uploads what was picked, so the button stays busy for it
    setBusy(true);
    setFailure(null);
    let picked: string | null = null;
    try {
      picked = await pickArtifact('daemon');
    } catch (reason) {
      setFailure(describeError(reason));
    } finally {
      setBusy(false);
  ...
  return (
    <Section>
      <SectionHeader title="push a local build" hint={`a daemon binary or .swu from ${hostOrigin()}`} />
