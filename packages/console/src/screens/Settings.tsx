...
import { useConsole } from '../console.ts';
import { host as adapter, selfUpdate, type Lifecycle } from '../host.tsx';
    ...
    <Screen>
      ...
      <Section>
        ...
        <ListGroup>
          <ListRow
            icon={<Icon name="info" />}
            title={host?.appName ?? 'bridgething'}
...
function ThisApp(): VNode {
  // a host that cannot start itself or stop itself leaves those rows out
  // entirely, rather than showing a control that answers nothing
  const { autostart: startsWithMachine, lifecycle } = adapter();

  return (
    <Section>
      <SectionHeader title="this app" />
      <ListGroup>
        {startsWithMachine ? <AutostartRow /> : null}
        <DebugLoggingRow />
        {lifecycle ? <QuitRow lifecycle={lifecycle} /> : null}
      </ListGroup>
    </Section>
  );
}
...
function QuitRow({ lifecycle }: { lifecycle: Lifecycle }): VNode {
  return (
    <ListRow
