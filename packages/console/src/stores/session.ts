...
import type { ConsoleSession, KnownDevice } from '../console.ts';
import { host } from '../host.tsx';

import { autostart } from './autostart.ts';
import { keyed, resource, type Store } from './resource.ts';

let attached: ConsoleSession | null = null;

function bound(): ConsoleSession {
...
export async function seed(session: ConsoleSession): Promise<void> {
  attached = session;
  ...
  void Promise.all([
    ...
    host().autostart ? autostart.refresh() : Promise.resolve(),
  ]);
