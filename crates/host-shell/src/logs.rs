...
const DEFAULT_FILTER: &str = "warn,bridgething_desktop=debug,bridgething_headless=debug,\
bridgething_host_shell=debug,bridgething_companion=debug,bridgething_delivery=debug,\
bridgething_gateway=info,bridgething=info";
const VERBOSE_FILTER: &str = "warn,bridgething_desktop=trace,bridgething_headless=trace,\
bridgething_host_shell=trace,bridgething_companion=trace,bridgething_delivery=trace,\
bridgething_gateway=trace,bridgething=trace";
...
/// Stands up the subscriber both host surfaces share: a swappable filter, a
/// stderr writer, and the ring the logs screen reads.
pub fn install() -> Arc<Verbosity> {
  use tracing_subscriber::{layer::SubscriberExt as _, util::SubscriberInitExt as _};

  let (filter, reload) = reload::Layer::new(filter(false));
  tracing_subscriber::registry()
    .with(filter)
    .with(tracing_subscriber::fmt::layer().with_writer(std::io::stderr))
    .with(RingLayer)
    .init();
  Arc::new(Verbosity::new(reload))
}
...
impl Verbosity {
  ...
  /// A handle that answers but swaps nothing, for a surface that runs without
