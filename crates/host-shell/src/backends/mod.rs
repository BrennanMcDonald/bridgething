#[cfg(all(feature = "voice", any(target_os = "linux", target_os = "macos", target_os = "windows")))]
mod asr;
#[cfg(all(feature = "desktop-session", any(target_os = "linux", target_os = "macos", target_os = "windows")))]
mod geo;
#[cfg(target_os = "linux")]
mod linux;
#[cfg(target_os = "macos")]
...
#[cfg(all(
  feature = "voice",
  any(target_os = "linux", target_os = "windows", all(target_os = "macos", test))
))]
mod nlu;
...
#[cfg(all(feature = "voice", any(target_os = "linux", target_os = "macos", target_os = "windows")))]
pub(crate) fn voice(paths: &ModelPaths, platform: ModelPlatform) -> Voice {
  Voice {
    speech: Some(Arc::new(asr::WhisperSpeech::new(paths.clone()))),
    #[cfg(target_os = "macos")]
    nlu: Some(Arc::new(macos::nlu::CoreMlNlu::new(paths.clone()))),
    #[cfg(not(target_os = "macos"))]
    nlu: Some(Arc::new(nlu::OrtNlu::new(paths.clone()))),
    model_validator: Some(Arc::new(models::LocalArtifactValidator)),
    model_platform: Some(platform),
  }
}
...
pub(crate) fn voice(_paths: &ModelPaths, _platform: ModelPlatform) -> Voice {
  Voice::default()
}
