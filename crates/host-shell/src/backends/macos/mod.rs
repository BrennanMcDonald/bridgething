...
use crate::backends::{ModelPaths, Platform, voice};

pub fn platform() -> Platform {
  let paths = ModelPaths::default();
  let voice = voice(&paths, ModelPlatform::Macos);
  Platform {
    #[cfg(feature = "desktop-session")]
...
#[cfg(all(test, feature = "voice", feature = "desktop-session"))]
mod tests {
  ...
  fn the_validator_takes_a_published_bundle_and_refuses_bytes_that_are_not_one() {
    let validator = LocalArtifactValidator;
